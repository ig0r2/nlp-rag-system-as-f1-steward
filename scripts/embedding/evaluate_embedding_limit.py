...
MAX_DISTANCE = 0.6  # or None to disable
...
def evaluate_all_ks(all_docs, ks, collection, max_distance):
    ...
    for i in range(len(metadatas)):
        ...
        if max_distance is not None:
            distances = [
                d for d, id_ in zip(results["distances"][0], results["ids"][0]) if id_ != ids[i]
            ][:max_k]
            neighbors = [n for n, d in zip(neighbors, distances) if d <= max_distance]
    ...
    metrics_by_k = {}
    for k in ks:
        precision_arr, recall_arr, hitrates_any, hitrates_all = [], [], [], []
...
def plot_results(results_by_collection: dict, ks: list):
    metric_keys = ["precision", "recall", "hitrate_any", "hitrate_all"]
    metric_labels = ["Precision", "Recall", "Hit Rate Any", "Hit Rate All"]
    file_names = ["precision.png", "recall.png", "hitrate_any.png", "hitrate_all.png"]

    save_basename = get_results_path() / "embedding_limit"
...
if __name__ == "__main__":
    ...
    with Logger(get_logs_path() / "embedding/evaluate.txt"):
        ...
        for db in COLLECTIONS_All:
            ...
            raw_evaluations[db.name] = evaluate_all_ks(docs, K_All, collection, max_distance=MAX_DISTANCE)
